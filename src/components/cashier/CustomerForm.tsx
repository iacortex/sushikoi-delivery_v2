...
export const CustomerForm: React.FC<CustomerFormProps> = ({
  ...
  // Geocoding hook
  const { geocode, isLoading: isGeocoding, error: geocodingError } = useGeocoding();
  ...
  useEffect(() => {
    ...
    performGeocoding();
  }, [debouncedGeocodingKey, geocode, onCoordsChange]);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      {/* Geocoding Status */}
      {(isGeocoding || geocodingError || geocodedResult) && (
        <div className="card">
          <div className="card-body">
            {isGeocoding && (
            ...
            {geocodingError && (
              <div className="flex items-center gap-2 text-red-600">
                <AlertCircle size={16} />
                <span>Error al buscar ubicación: {geocodingError}</span>
              </div>
            )}
            
            {geocodedResult && !isGeocoding && (
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-green-600">
                  <MapPin size={16} />
                  <span>Ubicación encontrada</span>
                  {geocodedResult.precision !== 'exact' && (
                    <span className="text-xs px-2 py-1 rounded bg-amber-50 text-amber-800">
                      Precisión: {geocodedResult.precision}
                    </span>
                  )}
                </div>
                ...
                {geocodedResult.precision !== 'exact' && (
                  <p className="text-sm text-amber-700">
                    ⚠️ No se encontró la numeración exacta. Por favor, ajusta la ubicación en el mapa.
                  </p>
                )}
              </div>
            )}
        ...
        <div className="card">
          ...
          <div className="card-body">
            ...
            <div className="mt-3 text-xs text-gray-500 space-y-1">
              <p>📍 Coordenadas: {selectedCoords.lat.toFixed(6)}, {selectedCoords.lng.toFixed(6)}</p>
              {geocodedResult && (
                <p>🎯 Precisión: {geocodedResult.precision} 
                  {geocodedResult.matchedNumber && " (numeración exacta)"}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="card-body">
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
              ...
              <div className="border-t border-green-300 pt-3 space-y-1">
                ...
                {selectedCoords && geocodedResult && (
                  <div className="flex justify-between text-sm text-green-600">
                    <span>Ubicación:</span>
                    <span>Confirmada ({geocodedResult.precision})</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {!canCreateOrder && (
              <div className="mt-3 text-sm text-gray-500 space-y-1">
                <p className="text-center font-medium">Para crear el pedido necesitas:</p>
                <ul className="text-xs space-y-1">
                  {!hasCartItems && <li>• Agregar productos al carrito</li>}
                  {!customerData.name && <li>• Nombre del cliente</li>}
                  {!customerData.phone && <li>• Teléfono del cliente</li>}
                  {!customerData.street && <li>• Dirección (calle)</li>}
                  {!customerData.number && <li>• Número de dirección</li>}
                  {!selectedCoords && <li>• Confirmar ubicación en el mapa</li>}
                </ul>
              </div>
            )}
          </div>
