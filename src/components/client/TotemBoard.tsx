...
interface TotemBoardProps {
  orders: Order[];
}

export const TotemBoard: React.FC<TotemBoardProps> = ({ orders }) => {
  // Filter active orders (not delivered) and sort by creation time
  const activeOrders = useMemo(() => {
    return orders
      .filter(order => order.status !== 'delivered')
      .sort((a, b) => b.createdAt - a.createdAt)
      .slice(0, 12); // Show maximum 12 orders on screen
  }, [orders]);
    ...
    <div className="bg-gradient-to-br from-gray-900 to-gray-800 text-white rounded-xl p-6 min-h-[700px]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {activeOrders.map((order) => (
          <TotemOrderCard key={order.id} order={order} />
        ))}
      </div>
      ...
      <div className="mt-6 pt-4 border-t border-gray-700 flex items-center justify-between text-sm text-gray-400">
        ...
        <div>
          <span>🍣 Sushi Delivery Puerto Montt • Sistema de seguimiento</span>
        </div>
      </div>
    </div>
  );
...
// Individual order card for totem display
interface TotemOrderCardProps {
  order: Order;
}

const TotemOrderCard: React.FC<TotemOrderCardProps> = ({ order }) => {
  const progress = useMemo(() => progressFor(order), [order]);
  const minutesLeft = useMemo(() => minutesLeftFor(order), [order]);

  // Status configuration for totem display
  ...
  return (
    <div className={`
      ${statusConfig.bgColor} border-2 rounded-lg p-4 
    ...
    `}>
      ...
      <div className="mb-3">
        ...
        <p className="font-medium text-white truncate" title={order.name}>
          {order.name}
        </p>
      </div>
      ...
      <div className="text-center">
        ...
        <div className="bg-white p-2 rounded-lg inline-block">
          <img
            src={getTrackingQRUrl(shortCode(order.id), 80)}
            alt={`QR ${shortCode(order.id)}`}
            className="w-16 h-16"
          />
        </div>
      </div>
