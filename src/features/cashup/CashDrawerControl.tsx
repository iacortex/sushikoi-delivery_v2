...
import { getCashupCompat, fallbackExpectedCash } from "./compat";

const fmt = (n: number) =>
  new Intl.NumberFormat("es-CL").format(Math.round(n || 0));

// 👇 lista ampliada (mantengo los que ya tenías)
const CASHIERS = [
  "Camila Guzmán",
  "Francisco Kuchel",
  "Camila Yañez",
...
const CashDrawerControl: React.FC = () => {
  const raw = useCashup() as any;
  const cash = getCashupCompat(raw);
  ...
  const expected =
    (cash.getExpectedCash && cash.getExpectedCash(current)) ||
    fallbackExpectedCash(current);

  const lastClosed = React.useMemo(
    () => (cash.listSessions() || []).filter((s: any) => s?.status === "CLOSED").slice(0, 5),
    // reevalúa cuando cambia el turno
    [current]
  );
  ...
  const handleOpen = () => {
    if (cash.openSession) cash.openSession(resolveName());
    else console.warn("[CashDrawerControl] openSession no implementado");
  };

  const handleClose = () => {
    const c = Number(String(counted).replace(/[^\d]/g, "")) || 0;
    if (!isOpen) return;
    if (cash.closeSession) {
      const closed = cash.closeSession(c);
      if (closed) setClosedForPrint(closed);
    } else {
      console.warn("[CashDrawerControl] closeSession no implementado");
    }
    setCounted("");
  };
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div className="text-sm">
          {isOpen ? (
            <span className="inline-flex items-center gap-2 px-2 py-1 rounded bg-green-50 text-green-700 border border-green-200">
              <LockOpen size={14} /> Abierta —{" "}
              {current?.open?.cashierName || "Cajero"}
            </span>
          ) : (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {lastClosed.map((s: any) => {
                      const close = s?.close || {};
                      const expectedCash = pick<number>(
                        close,
                        ["expectedCash", "expected", "expected_amount"],
                        0
                      );
                      const countedCash = pick<number>(
                        close,
                        ["countedCash", "counted", "cash_counted"],
                        0
                      );
                      const diff =
                      ...
                      return (
                        <tr
                          key={s.id || String(close.closedAt || Math.random())}
                          className="border-t"
                        >
                          <td className="py-1">
                            {new Date(
      ...
      {closedForPrint && (
        <CloseReport80mm
          session={closedForPrint}
          onClose={() => setClosedForPrint(null)}
        />
      )}
    </div>
  );
